...
def train(model, train_loader, criterion, optimizer, device, note_data):
    ...
    for inputs, (targets_note, targets_offset, targets_duration) in train_loader:
        inputs, targets_note, targets_offset, targets_duration = inputs.to(device), targets_note.to(device), targets_offset.to(device), targets_duration.to(device)
...
def main():
   ...
   # Load note data and network data
   ...
   #      note_data, network_data = pickle.load(f)
    ...
    model = ConvLSTM(note_data)
    model = model.to(device)

    # Define the criterion and optimizer
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(model.parameters(), lr=1e-6, amsgrad=True)

    # Define parameters for saving and generating MIDI
    save_interval = 50  # Save every 50 epochs
    generate_interval = 25  # Generate MIDI every 10 epochs
